...
void WiegandOut::createParity(unsigned long data, unsigned int bits, bool useFacilityCode)
{
   ...
   for (int i = 0; i < b; i++)
   {
      ...
      tempData = data >> i;
      if (tempData & 0x01)
      { // this count the qty of 1 and do a xor to complete the even parity;
   ...
   for (int i = b; i < bits - 2; i++)
   {
      tempData = data >> i;
      if (tempData & 0x01)
      { // this count the qty of 1 and do a xor to complete the odd parity;
